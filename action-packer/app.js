...
// restored immediately afterwards. Export therefore keeps the
...
async function exportFBX() {
  ...
  try {
    ...
    setStatus(
      'FBX exportado con bind pose reconstruido y orientación específica del destino (' +
...
async function exportGLB() {
  ...
  try {
    ...
    setStatus('GLB exportado con la misma reconstrucción de rest pose: ' + filename + ' · ' + clips.length + ' actions.', 'ok');
  } catch (error) {
    console.error(error);
