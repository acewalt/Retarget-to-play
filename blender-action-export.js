...
export function buildOriginalRigDeltaData(
  ...
  const hasRigifyIk = [...records.values()].some(record =>
    /^(hand_ik|foot_ik|upper_arm_ik_target|thigh_ik_target)\.[LR]$/.test(record.bone)
  );

  const hasRigify = hasRigifyIk || [...records.values()].some(record =>
    /^(upper_arm_fk|forearm_fk|hand_fk|thigh_fk|shin_fk|foot_fk)\.[LR]$/.test(record.bone)
  );
...
# CloudRig: ik_*=0/1. Rigify: IK_FK=0/1 and pole_vector=True for
# the pole-based bake generated by Retarget-to-play.
    ...
    for key in list(pb.keys()):
        ...
        if USE_RIGIFY and key == 'IK_FK' and isinstance(value, (int, float)):
            if float(value) != RIGIFY_IK_VALUE:
                ...
                changed_props.append(pb.name + ':IK_FK=' + str(RIGIFY_IK_VALUE))
            continue

        if USE_RIGIFY_IK and key == 'pole_vector':
            desired = True
            if bool(value) != desired:
                pb[key] = desired
                changed_props.append(pb.name + ':pole_vector=True')
            continue
...
if USE_RIGIFY:
    print('[Retarget-to-play] Rigify modo:', 'IK' if USE_RIGIFY_IK else 'FK', '· pole_vector:', USE_RIGIFY_IK)
else:
    print('[Retarget-to-play] CloudRig modo:', 'IK' if USE_IK else 'FK')
print('[Retarget-to-play] props cambiadas:', len(changed_props))
for item in changed_props:
    print('  ', item)
`;

  return [
    '# Retarget-to-play - ORIGINAL RIG ACTION',
    '# Pose-basis portable para CloudRig / Rigify original.',
    '',
    'import bpy',
    'import json',
